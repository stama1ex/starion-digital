...
export const AR_TRACKING_OPTIONS = {
  // cutOff = filterMinCF + filterBeta * |скорость|, и чем ниже cutOff, тем
  // сильнее сглаживание. В покое скорость около нуля, то есть всё решает
  // filterMinCF — это и есть главная ручка против дрожания на неподвижном
  // маркере. Я держал его на 0.5 (фактически фильтр выключен), рассчитывая
  // сгладить всё самостоятельно; это была ошибка: OneEuroFilter адаптивный и
  // с шумом оценки справляется лучше простого сглаживания. Возвращаем близко
  // к дефолту MindAR, а мёртвая зона ниже добирает остаток.
  filterMinCF: 0.0015,
  // Отвечает за скорость при движении: чем выше, тем меньше отставание, когда
  // сувенир двигают. Ниже прежних 50000, но заметно выше дефолта (1000).
  filterBeta: 6000,
  // Сколько кадров подряд маркер должен теряться, прежде чем движок сочтёт его
  // потерянным. Дефолт 5 — контент срывался от блика, наклона и смаза.
  missTolerance: 40,
  // Сколько кадров нужно, чтобы признать маркер найденным.
  warmupTolerance: 3,
};

// Наш слой поверх фильтра движка. Он добирает то, чего OneEuroFilter не умеет:
// полностью игнорировать микродвижение и переживать короткие потери маркера.
export const AR_STABILIZER = {
  // Мёртвая зона. Пока маркер «шевелится» меньше порога, контент не двигается
  // вообще. В долях ширины маркера и в радианах: 0.016 ≈ 1.6 % ширины (на
  // магните 5 см это меньше миллиметра), 0.022 ≈ 1.3°.
  posDeadzone: 0.016,
  rotDeadzone: 0.022,
  // Насколько быстро контент догоняет маркер, доля на кадр.
  followMin: 0.09,
  // Поворот сглаживаем сильнее сдвига. При взгляде под углом оценка наклона
  // самая неустойчивая: положение центра держится, а плоскость «качает» —
  // именно это заметно как дрожание при съёмке сверху.
  followMinRot: 0.05,
  followMax: 1,
  speedGain: 11,
  // Сколько кадров держать последнюю позу после потери маркера. 45 кадров —
  // около 0.7 с. Дольше делать не стоит: контент начнёт заметно висеть уже
  // после того, как камеру увели в сторону.
  holdFrames: 45,
