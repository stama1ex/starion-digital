...
export const AR_TRACKING_OPTIONS = {
  ...
  filterMinCF: 0.001,
  // ЕДИНИЦЫ. Ровно та же ловушка, что уже съела наш стабилизатор: величины
  // MindAR не нормированы. Фильтр движка стоит на сырой матрице model-view
  // (Controller._glModelViewMatrix): элементы поворота там по модулю <= 1, а
  // перенос — в пикселях маркера, то есть сотни и тысячи. Время фильтр берёт
  // из Date.now(), поэтому «скорость» здесь — единицы в МИЛЛИСЕКУНДУ, а
  // сглаживание за кадр = 2*pi*cutOff*dt / (2*pi*cutOff*dt + 1).
  //
  // Отсюда порядок величин. Дрожание поворота порядка 0.5° за кадр — это
  // ~0.01 в элементе матрицы, за 40 мс ~2.5e-4 ед/мс. При filterBeta 5000
  // cutOff = 1.25, сглаживание = 0.997 — фильтр движка был сквозным
  // проводом. Дефолт mind-ar (1000) ничем не лучше: он рассчитан на
  // нормированные величины, которых здесь нет.
  //
  // При 3 то же дрожание даёт cutOff 0.0018 и сглаживание 0.31 (шум режется
  // втрое), а настоящий поворот сувенира 90°/с — 0.6, то есть контент за
  // рукой успевает. Перенос при таком beta фильтр почти не трогает, и это
  // правильно: сдвиг в плоскости маркера гомография считает точно, дрожит
  // наклон, а им занимается именно поворотная часть матрицы.
  //
  // Подбирать это можно только на живом телефоне: /ar/{slug}?arbeta=N.
  filterBeta: 3,
...
export const AR_STABILIZER = {
  // Доли сближения за кадр. Подобраны уже на работающем стабилизаторе:
  // измерения показали, что сырая поза спокойная (шаг наклона 0.03° за кадр,
  // ско 0.55°), поэтому душить её сильно незачем — важнее не отставать.
  //
  // Сдвиг в плоскости маркера — часть позы, определённая надёжно.
  followMin: 0.3,
  // Поворот в плоскости маркера: тоже надёжен, можно следовать быстро.
  followTwist: 0.3,
  // Наклон плоскости и глубина шумят сильнее, поэтому втрое медленнее — но
  // прежние 0.022 давали постоянную секунду на догон, что и читалось как
  // «картинка догоняет».
  followSwing: 0.1,
  followDepth: 0.1,
  followMax: 1,
  speedGain: 14,
  // Насколько движение ускоряет следование. Раньше разгон считался от
  // мгновенного расхождения, то есть наполовину от шума, и держать его
  // приходилось слабым — иначе шум сам себя и пропускал. Теперь на входе
  // разгона сглаженная скорость самой цели (см. speedSmooth), шума в ней
  // почти нет, и для наклона с глубиной разгон можно поднять: в покое
  // сглаживание остаётся сильным, а на реальном повороте сувенира контент
  // догоняет за 3-4 кадра, а не за десяток.
  boostFast: 6,
  boostSlow: 24,
  // Видео лежит в плоскости маркера, поэтому сдвиг и поворот в плоскости у
  // него ведём быстро: там гомография кладёт картинку точно, и запаздывание
  // читалось бы как сползание с сувенира.
  videoFollow: 0.55,
  // А вот наклон и глубину сглаживаем заметно — они берутся из разложения
  // гомографии, погрешность которого гуляет по кадрам, и именно она
  // проявляется как подрагивание. Втрое сильнее прежнего, но всё ещё втрое
  // мягче, чем у 3D: плоскому контенту столько демпфирования не нужно.
  videoSwing: 0.25,
  videoDepth: 0.25,
  // Сколько кадров держать последнюю позу после потери маркера (~0.7 c).
  holdFrames: 45,

  // --- защита от переворота плоскости ---
  //
  // Разложение гомографии плоскости даёт два решения: две ориентации, дающие
  // почти одинаковую картинку. При взгляде строго напротив они совпадают, при
  // сильном наклоне расходятся и выбор однозначен. А вот при малом наклоне —
  // в единицы градусов — они почти неразличимы, и оценка начинает скакать
  // между ними каждый кадр. Это и есть «повернул на пару градусов и сразу
  // затрясло». Сглаживанием такое не лечится: оценка не шумит, она прыгает
  // между двумя разными верными ответами, а их среднее — качание.
  //
  // Отличить переворот от настоящего наклона можно так: при перевороте наклон
  // скачет, а положение и поворот в плоскости остаются на месте. Физически так
  // не бывает — сувенир не наклоняется на 7° за один кадр, не сдвинувшись.
  flipAngle: 0.12, // рад (~7°) скачка наклона за кадр
  flipPos: 0.02, // при сдвиге меньше 2 % ширины маркера
  flipTwist: 0.05, // и повороте в плоскости меньше ~3°
  // Если «переворот» держится столько кадров подряд, значит сувенир правда
  // повернули — принимаем новую ориентацию.
  flipPatience: 14,

  // --- разгон следования ведём по СГЛАЖЕННОЙ скорости цели ---
  //
  // Раньше разгон считался от мгновенного расхождения позы с целью. Но при
  // неподвижном сувенире это расхождение и есть шум: он сам открывал
  // разгону дверь и сам же через неё проходил. Шум разнонаправлен, поэтому
  // усредняется вектор скорости, а не его длина: у настоящего движения
  // направление держится и длина сохраняется, у дрожания — гасится.
  speedSmooth: 0.25,
  // Догонять по расхождению всё-таки нужно: без этого отставание ничем не
  // ограничено и превращается в «картинка едет следом». Но расхождение при
  // неподвижном сувенире — это шум, поэтому в разгон идёт только та его
  // часть, которая шумом уже не объясняется. Порог не создаёт автоколебаний:
  // он стоит на входе РАЗГОНА, а не следования — внутри порога контент всё
  // равно едет к цели с базовой скоростью, просто без ускорения.
  driftFree: 0.02, // доли ширины маркера
  angFree: 0.035, // рад (~2°) — столько же для поворота

  // Постоянная времени показа, мс. Фильтр шагает по кадрам ТРЕКИНГА (движок
  // обновляет позу заметно реже, чем идёт отрисовка), а рисуем мы каждый
  // кадр — без этого сглаживания движение шло бы ступеньками с частотой
  // трекинга. Держим коротким: это не фильтр шума, а только развёртка
  // ступенек.
  viewTau: 30,
};

// Опции трекинга с учётом переопределений из адреса — подбирать filterBeta и
// filterMinCF можно только на реальном телефоне с реальным сувениром: на
// вебке ноутбука поза заметно спокойнее, и настройки, снятые там, на телефоне
// не работают. /ar/{slug}?arbeta=3&armincf=0.001
export function arTrackingOptions() {
  const opts = { ...AR_TRACKING_OPTIONS };
  ...
  const beta = num('arbeta');
  if (beta !== null) opts.filterBeta = beta;
  const minCf = num('armincf');
  if (minCf !== null) opts.filterMinCF = minCf;
  return opts;
}
