...
export const AR_TRACKING_OPTIONS = {
  // Дрожание на неподвижном маркере даёт именно filterMinCF: это сглаживание
  // в покое, и 0.02 (в 20 раз выше дефолта MindAR) заметно трясло картинку.
  // Снижаем до 0.005 — стоять станет спокойнее. filterBeta оставляем высоким:
  // он отвечает за скорость при движении, а отставание мы уже лечили именно им.
  filterMinCF: 0.005,
  filterBeta: 10000,
  // Сколько кадров подряд маркер должен теряться, прежде чем считать его
  // потерянным. Дефолт 5 — при бликах и наклоне контент срывался слишком
  // охотно; 12 кадров это примерно полсекунды, за них рука успевает вернуться.
  missTolerance: 12,
  // И наоборот: сколько кадров нужно, чтобы признать маркер найденным.
  // Меньше дефолта — контент появляется быстрее.
  warmupTolerance: 3,
};
