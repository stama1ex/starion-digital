...
// База для коротких ссылок в QR (/a/{code}). Домен в QR спрятать нельзя —
// это обычная ссылка, — но его можно сделать нейтральным: направьте на это же
// приложение отдельный домен и укажите его здесь, тогда на сувенирах для
// сторонних компаний не будет светиться основной адрес. Пусто — берётся origin
// страницы, на которой генерируется QR.
