...
export const AR_STABILIZER = {
  // Базовая доля сближения за кадр для положения.
  followMin: 0.14,
  // Поворот демпфируем сильнее. Для 3D это главное: ошибка наклона плоскости
  // умножается на высоту модели, и то, что для лежащего на маркере видео
  // почти незаметно, у стоящей фигуры смещает голову на сантиметры.
  followMinRot: 0.06,
  followMax: 1,
  // Во сколько раз расхождение ускоряет сближение. Подобрано так, чтобы
  // заметное движение руки проходило за 2-3 кадра.
  speedGain: 14,
  // Сколько кадров держать последнюю позу после потери маркера.
  // 45 кадров ≈ 0.7 с — переживает наклон и блик, но контент ещё не «висит»
  // после того, как камеру увели.
  holdFrames: 45,
};
