...
// Дефолты MindAR: filterMinCF 0.001, filterBeta 1000. Здесь подняты, чтобы
// контент быстрее «догонял» маркер при движении/повороте:
//  - filterMinCF — базовая частота среза: выше => меньше лага в покое,
//    чуть больше микро-дрожания;
//  - filterBeta — насколько фильтр ускоряется при движении: выше => контент
//    почти не отстаёт, когда маркер быстро двигают/крутят.
// (раньше здесь стояло filterBeta: 0.001 — в миллион раз ниже дефолта, отсюда
// и «ползёт следом».)
