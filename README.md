...
- **QR domain** is `NEXT_PUBLIC_AR_SHORT_URL` when set: souvenirs are also made
  under other brands, so the app can be served from a separate domain attached to
  the same Vercel project (an alias, not a redirect) and the main address never
  appears in the QR.
