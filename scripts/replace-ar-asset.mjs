...
//
// Старый файл не удаляется: он остаётся в R2 под прежним ключом, поэтому
// откатиться можно, вернув прежний путь в базе. Убрать его потом —
// scripts/sweep-ar-orphans.mjs.
//
// Запуск:
